...
for i in range(len(content_paths)):
    ...
    if content_actions[i] =='get' and (args['methods']=='All' or args['methods']=='Get'):
        get = SqlGet()
        get.set_logger(log)
        #get.set_connection_string('mssql+pyodbc://mhcps/centricity?driver=SQL+Server')
        get.set_connection_string('mssql+pyodbc://script:UtIK3g0gZTKa9Z@mhcps/centricity?driver=SQL+Server')
    ...
    if content_actions[i] =='process' and (args['methods']=='All' or args['methods']=='Process'):
        ...
        #process.set_connection_string('DRIVER={SQL Server};SERVER=mhcps;DATABASE=centricity;Trusted_Connection=no;')
        process.set_connection_string('DRIVER={SQL Server};SERVER=mhcps;DATABASE=centricity;Trusted_Connection=no;UID=script;PWD=UtIK3g0gZTKa9Z')
