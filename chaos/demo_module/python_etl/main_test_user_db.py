...
'''
    ...
    TODO: Dynamic INI for Server names
    TODO: Dynamic names for local DB names.
'''
...
for i in range(len(content_paths)):
    ...
    if content_actions[i] =='get' and (args['methods']=='All' or args['methods']=='Get'):
        get = SqlGet()
        get.set_logger(log)
        get.set_connection_string('mssql+pyodbc://MHKM/user_' + str(os.getlogin()).lower() + '?driver=SQL+Server')
    ...
    if content_actions[i] =='process' and (args['methods']=='All' or args['methods']=='Process'):
        ...
        process.set_connection_string('DRIVER={SQL Server};SERVER=MHKM;DATABASE=user_' + str(os.getlogin()).lower() + ';Trusted_Connection=yes;')
