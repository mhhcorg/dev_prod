...
for i in range(len(content_paths)):
    ...
    if content_actions[i] =='get' and (args['methods']=='All' or args['methods']=='Get'):
        get = SqlGet()
        get.set_logger(log)
        get.set_connection_string('mssql+pyodbc://MHKM/dw_temp?driver=SQL+Server')
    ...
    if content_actions[i] =='process' and (args['methods']=='All' or args['methods']=='Process'):
        ...
        process.set_connection_string('DRIVER={SQL Server};SERVER=MHKM;DATABASE=dw_temp;Trusted_Connection=yes;')
