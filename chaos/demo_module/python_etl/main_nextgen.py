...
'''
    ...
    2023-05-24 - Updated to use the new connection string for the new get process.
'''
...
log = StandardLogging().set_level_debug().get_run_logger(args['base_path'])
...
for i in range(len(content_paths)):
    ...
    if content_actions[i] =='get' and (args['methods']=='All' or args['methods']=='Get'):
        get = SqlGet()
        get.set_logger(log,'DEBUG')
        get.set_connection_string('DRIVER={SQL Server};SERVER=MHKM;DATABASE=dw_staging;Trusted_Connection=yes;')
...
log.info('Batch Done: ' + str(datetime.timedelta(seconds=round(time.time() - start_time,3))))